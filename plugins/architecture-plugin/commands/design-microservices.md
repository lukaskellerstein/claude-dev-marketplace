...
2. **Launch Microservices Architect**: Use the `architecture-plugin:microservices-architect` agent to:
...
3. **Validate with Cloud Patterns**: Use the `architecture-plugin:cloud-patterns-expert` agent to:
...
4. **Event-Driven Integration** (if applicable): If the system requires asynchronous communication, use the `architecture-plugin:event-sourcing-expert` agent to:
